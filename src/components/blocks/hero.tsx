...
export interface HeroProps extends Omit<React.HTMLAttributes<HTMLElement>, "title"> {
  ...
  actions?: { label: string; href?: string; onClick?: () => void; variant?: "default" | "destructive" | "outline" | "secondary" | "ghost" | "link" }[];
  ...
  (
    ...
    return (
      <section
        ref={ref}
        className={cn(
          "relative z-0 flex min-h-screen w-full flex-col items-center justify-center overflow-hidden rounded-md bg-transparent",
          className,
        )}
        {...props}
      >
